...
public class Greedy_5 {
    ...
    인접한 두 도시 사이의 도로들은 서로 길이가 다를 수 있다. 도로 길이의 단위는 km를 사용한다. 처음 출발할 때 자동차에는 기름이 없어서
    주유소에서 기름을 넣고 출발하여야 한다. 기름통의 크기는 무제한이어서 얼마든지 많은 기름을 넣을 수 있다. 도로를 이용하여 이동할 때 1km마다 1리터의 기름을 사용한다.
    ...
    다음 도시에서 1리터의 기름을 넣어(1×4 = 4원) 제일 오른쪽 도시로 이동하면, 총 비용은 20원이다. 또 다른 방법으로 제일 왼쪽 도시에서 2리터의 기름을 넣고(2×5 = 10원)
    ...
    public static void greedy() throws IOException {
        ...
        // 거리 배열 기준으로 계산해야 하므로 N - 1을 함, dist.length를 사용해도 무방
        for(int i=0; i < N - 1; i++){
            // 현제 기름값이 이전 기름값보다 쌀 경우 갱신
            if(cost[i] < minCost){
                minCost = cost[i];
            }
            // 갱신되지 않으면 기존 기름값과 거리를 곱함
            sum += (minCost * dist[i]);
        }

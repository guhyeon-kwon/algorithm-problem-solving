...
public class BigBody {
    ...
    A의 덩치가 B보다 큰 셈이 된다. 그런데 서로 다른 덩치끼리 크기를 정할 수 없는 경우도 있다. 예를 들어 두 사람 C와 D의 덩치가 각각 (45, 181), (55, 173)이라면
    ...
    그 사람의 덩치 등수는 k+1이 된다. 이렇게 등수를 결정하면 같은 덩치 등수를 가진 사람은 여러 명도 가능하다. 아래는 5명으로 이루어진 집단에서
    각 사람의 덩치와 그 등수가 표시된 표이다.
    이름	(몸무게, 키)	덩치 등수
    A	(55, 185)	2
    B	(58, 183)	2
    C	(88, 186)	1
    ...
    public static void solve() throws IOException {
        ...
        for(int i=0; i<N; i++){
            ...
            for(int j=0; j < N; j++){
                if(i == j) continue; // i와 j가 같으면 자기자신이므로 비교하지 않음
